...
class TestServerFunctionality(TestBase):
    ...
    def test_play_with_invalid_append_args(self):
        ...
        request["testId"] = "foo"
        request["services"] = [
            {"serviceName": "PLAY_NEXUS_END_TO_END_TEST", "runFrom": "SNAPSHOT", "appendArgs": "-Dshould=be-an-array",}
        ]
    ...
    def test_offline(self):
        ...
        self.assertEqual(context.get_service("TEST_ONE").status(), [])
        request["testId"] = "foo2"
        smserverlogic.SmStartRequest(server, request, True, True, False, True).process_request()
    ...
    def test_ensure_multiple_instances_of_a_service_can_be_started_from_server(self):
        ...
        smserverlogic.SmStartRequest(server, first_request, True, False, False, True).process_request()

        def single_service_started_successfully():
            if len(context.get_service("TEST_ONE").status()) != 1:
                return False
            if len(context.get_service("DROPWIZARD_NEXUS_END_TO_END_TEST").status()) != 1:
                return False
            if len(context.get_service("PLAY_NEXUS_END_TO_END_TEST").status()) != 1:
                return False
            return True
        ...
        smserverlogic.SmStartRequest(server, second_request, True, False, False, True).process_request()

        def multiple_services_started_successfully():
            if len(context.get_service("TEST_ONE").status()) != 2:
                return False
            if len(context.get_service("DROPWIZARD_NEXUS_END_TO_END_TEST").status()) != 2:
                return False
            if len(context.get_service("PLAY_NEXUS_END_TO_END_TEST").status()) != 2:
                return False
            return True
