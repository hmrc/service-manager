...
class TestSmStatus(unittest.TestCase):
    ...
    def mock_service(self, service_name, up):
        ...
        mock_service.status = lambda all_process: [SmServiceStatus(service_name, 123, 1, 123123, 512, 8080, "", "run-test-service " + service_name, "", SmServiceStatus.HEALTHCHECK_PASS)] if up else None
        return mock_service

    def setUp(self):
        self.context = MagicMock()
        self.context.services = lambda: [self.mock_service("test-service", True), self.mock_service("test-service-2", False)]
